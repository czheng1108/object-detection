...
from .losses import Diou_loss
from .losses import Iou_loss
from .losses import Ciou_loss
from .losses import Giou_loss
from .losses import ghm_c_loss
...
class RetinaNetClassificationHead(nn.Module):
    ...
    def compute_loss(self,
        ...
        for targets_per_img, cls_logits_per_img, matched_idxs_per_img in zip(targets, cls_logits, matched_idxs):
            ...
            # compute ghm_c loss
            # loss_ = ghm_c_loss(cls_logits_per_img[valid_idxs_per_img],
            #     gt_classes_target[valid_idxs_per_img])
            # losses.append(loss_ / max(1, num_foreground))
...
class RetinaNet(nn.Module):
    ...
    def compute_loss_atss(self, targets, head_outputs, anchors):
        # type: (List[Dict[str, Tensor]], Dict[str, Tensor], List[Tensor]) -> Dict[str, Tensor]
        """

        Parameters using atss to calculate loss
                   https://blog.csdn.net/chenzhoujian_/article/details/109166250
                   https://github.com/sfzhang15/ATSS/blob/master/atss_core/modeling/rpn/atss/loss.py
        ----------
        targets: the ground truth box
        head_outputs:the bounding box of prediction
        anchors: the set of anchors

        Returns: the model's loss(a float tensor)
        -------

        """
        # a quite robust hyperparameter with a default value of 9
        K = 9
