...
def create_model_using_trained_model(num_classes, device):
    print('using the weights of model-3.pth!')
...
def create_model(num_classes, device):
    # print('using the weights of pytorch!')
...
def main(parser_data):
    ...
    # create model num_classes equal background + 2 classes
    model = create_model(num_classes=parser_data.num_classes + 1, device=device)
    # model = create_model_using_trained_model(num_classes=parser_data.num_classes+1, device=device)
    # print(model)
    ...
    for epoch in range(parser_data.start_epoch, parser_data.epochs+parser_data.start_epoch):
        # train for one epoch, printing every 10 iterations
        mean_loss, lr = utils.train_one_epoch(model, optimizer, train_data_loader,
                                              device, epoch, print_freq=20, warmup=True)
